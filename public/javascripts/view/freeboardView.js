...
// 자유게시판 더보기 뷰 랜더링
export function renderFreeLoadMore(result, type){
    var freeBoard = "<div id='board-free-detail' class='card'>";
        freeBoard += "<div class='card card-price'>";
        freeBoard += "<div class='card card-default card-header'>";        
        freeBoard += "<ul class='details'>";            
        freeBoard += "<li class='price'><a id='board-free-title' class='pluslink'>자유게시판</a></li>";
        freeBoard += "</ul>";                     
        freeBoard += "</div>";                
        freeBoard += "<div class='table-wrap card-body'>";           
        freeBoard += "<table id='free-board-table' class='table table-hover table-sm'>";            
        freeBoard += "<thead>";
        freeBoard += "<tr class='card card-default card-header'>";
        freeBoard += "<th class='text-center col-md-7'>제목</th>";  
        freeBoard += "<th class='text-center col-md-2'>작성자</th>";
        freeBoard += "<th class='text-center col-md-2'>날짜</th>";
        freeBoard += "<th class='text-center col-md-1'>조회수</th>";   
        freeBoard += "</tr>";
        freeBoard += "</thead>";      
        freeBoard += "<tbody>";
        // 공지사항 영역
        freeBoard += "<tr class='notice-row-free'>";
        freeBoard += "<td class='text-center' id='notice-free-title'></td>";
        freeBoard += "<td class='text-center'><img id='notice-free-user-profile' class='tr-user-profile img-circle'></img><span id='notice-free-user-name'></span></td>";
        freeBoard += "<td class='text-center note' id='notice-free-timestamp'></td>";
        freeBoard += "<td class='text-center'></td>";
        freeBoard += "</tr>";   
         
        if(result !== undefined){
            if(!result.length){
                freeBoard += "<tr><td class='text-center' colspan='4'>게시글이 없습니다.</td></tr>";  
            }else{
                for(var i=0; i< result.length; i++){
                    ...
                    if(result[i].reply){
                        ...
                        if(result[i].uid === "unknown"){
                            freeBoard += "<tr value='"+ result[i].idx +"'>";
                            freeBoard += "<td>" + title + "<span class='badge'>" + badgeCount + "</span></td>";         
                            freeBoard += "<td class='text-center'><img class='tr-user-profile img-circle' id='"+ result[i].idx +"'></img>" + userNickname + "</td>";
                            freeBoard += "<td class='text-center note'>" + timestamp + "</td>";
                            freeBoard += "<td class='text-center note'>" + views + "</td>";
                            freeBoard += "</tr>";                 
                        }else{
                            freeBoard += "<tr value='"+ result[i].idx +"'>";
                            freeBoard += "<td>" + title + "<span class='badge'>" + badgeCount + "</span></td>";         
                            freeBoard += "<td class='text-center'><img class='tr-user-profile img-circle' id='"+ result[i].idx +"'></img>" + userNickname + + "<img class='profile' src='/images/icon-star.png'></img></td>";
                            freeBoard += "<td class='text-center note'>" + timestamp + "</td>";
                            freeBoard += "<td class='text-center note'>" + views + "</td>";
                            freeBoard += "</tr>";                 
                        }
        ...
        // 글쓰기 버튼
        freeBoard += "<div class='board-write'>";
        freeBoard += "<button type='submit' id='write-submit' class='btn btn-info btn-block'><i class='fas fa-pencil-alt'></i></i> 글쓰기 </button> ";
        freeBoard += "</div>";

        // 검색창
        freeBoard += "<div class='row divide'>";    
        freeBoard += "<div class='col-xs-8 col-xs-offset-2'>";
		freeBoard += "<div class='input-group'>";
        freeBoard += "<div class='input-group-btn search-panel dropup'>";
        freeBoard += "<button type='button' class='btn btn-default dropdown-toggle' data-toggle='dropdown'>";
        freeBoard += "<span id='search_concept'>전체</span> <span class='caret'></span>";
        freeBoard += "</button>";
        freeBoard += "<ul id='category' class='dropdown-menu' role='menu'>";
        freeBoard += "<li><a>전체</a></li>";       
        freeBoard += "<li><a>작성자</a></li>";
        freeBoard += "<li><a>제목</a></li>";
        freeBoard += "<li><a>날짜</a></li>";
        freeBoard += "<li><a>내용</a></li>";           
        freeBoard += "</ul>";
        freeBoard += "</div>";                
        freeBoard += "<input type='text' id='search_param' class='form-control' name='x'>";
        freeBoard += "<span class='input-group-btn'>";
        freeBoard += "<button id='search-go' class='btn btn-default' type='button'><span class='glyphicon glyphicon-search'></span></button>";
        freeBoard += "</span>";
        freeBoard += "</div>";
        freeBoard += "</div>";
        freeBoard += "</div>";     
        ...
        // 페이징 버튼
        if(type === 'paging'){
            freeBoard += "<div class='text-center'>";
            freeBoard += "<ul class='pagination'>";  
            freeBoard += "<li id='backward'><a>&lsaquo;</a></li>"                     
            freeBoard += "<li id='forward'><a>&rsaquo;</a></li>";                                                                                   
            freeBoard += "</ul>";
            freeBoard += "</div>";
        }
...
// 자유게시판 글 작성 에디터 랜더링
export function renderSummernoteFree(){
    var summernote = "<div id='board-container'>";
        ...
        summernote += "<li class='price'><a id='board-free-title' class='pluslink'>자유게시판</a></li>";     
        summernote += "</ul>";              
        summernote += "</div>";    
        ...
        summernote += "<button type='submit' id='write-board' class='btn btn-info'><i class='glyphicon glyphicon-pencil'></i> 글 작성 완료 </button>";
        summernote += "</div>";
        summernote += "</div>";
        summernote += "</div>";
...
// 자유게시판 글 내용 뷰 랜더링
export function renderFreeBoardDetail(result){
    var detailarea = "<div id='board-container'>";
        ...
        detailarea += "<li class='price'><a id='board-free-title' class='pluslink'>자유게시판</a></li>";     
        detailarea += "</ul>";              
        detailarea += "</div>";    
        ...
        if(result[0].uid === 'unknown'){
            ...
            detailarea += "<td class='text-left td-break-word'>";
            detailarea += "<img class='td-user-profile img-circle' id='"+ result[0].idx +"'></img>";
            detailarea += "<h6 class='userNickname'>"+ unescape(result[0].userNickname) +"<img class='profile' src='/images/icon-star.png'></img>";
            detailarea += "</h6><h6 class='timestamp pull-right'>"+ result[0].timestamp +"</h6>";
            detailarea += "</td>";  
        }
        detailarea += "</tr>";   
        detailarea += "<tr>";    
        ...
        detailarea += "<button type='submit' id='write-reply' class='btn btn-info'><i class='glyphicon glyphicon-pencil'></i> 댓글 작성 </button>";
        detailarea += "</div>";
        detailarea += "</div>";   
...
export function rednerEditPostSummernote(result){
    ...
    editPostArea += "<li class='price'><a id='board-free-title' class='pluslink'>자유게시판</a></li>";     
    editPostArea += "</ul>";              
    editPostArea += "</div>";    
