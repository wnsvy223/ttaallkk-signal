...
import * as freeBoardView from '../view/freeboardView.js';
...
//자유게시판 더 보기 뷰 세팅
function viewFreeBoard(result, isLastPage, recentKey, type){
    if(!result) return;
    ...
    // 뷰 태그 동적 생성 및 랜더링
    var freeBoard = freeBoardView.renderFreeLoadMore(result, type);
    $("#container").html(freeBoard); 
    ...
    //자유게시판 글 목록 클릭
    $("#free-board-table").on('click','tr',function(event){
        $(this).prop('disabled', true); 
        event.preventDefault();
        if(result.length){
    ...
    $("#free-board-table").find('tr:first').on('click', function(event) {
        event.preventDefault();
        event.stopPropagation(); 
    }); 

    // 테이블의 작성자 프로필사진 클릭시 게시글 내용으로 이동 방지
    $("#free-board-table tr").find('img').on('click', function(event) {
        event.preventDefault();
        event.stopPropagation();
    }); 
    ...
    $("#backward").on('click',function(event){ 
        if(result !== undefined && result.length){
            ...
            pagination("backward", key);
            var url = "/board/free/page?" + "endAt=" + key;
            utills.setPageHistory({key: key, url: url, type: "backward"}, url);      
        }   
    }); 
    ...
    $("#forward").on('click',function(event){                          
        if(result !== undefined && result.length){
            ...
            pagination("forward", key);
            var url = "/board/free/page?" + "startAt=" + key;
            utills.setPageHistory({key: key, url: url, type: "forward"}, url);
        }                       
    }); 
    redirectBoardFree();
...
function getSerchDataByCategory(node, value){
    boardDAO.getSerchValueOrderByCategory(consts.freeBoardRef, node, value).then(function(param){
        viewFreeBoard(param.result, false, param.recentKey, param.type);
    });
}
...
function getSerchDataByAll(value){
    var result = [];
    var ref = firebase.database().ref("board").child("free");
            ...
            ref.child(childSnapshot.key).orderByValue().startAt(value).endAt(value + "\uf8ff").once('value', function(data){
                if(data.val() !== null){
                    ref.child(data.key).once('value', function(searchPost){
                        ...
                        viewFreeBoard(param.result, false, param.recentKey, "search");
                    });          
                }
            });
        })
...
function getsummernote(){
        // 서머노트 에디터 랜더링
        var summernote = freeBoardView.renderSummernoteFree();
        ...
        //글 작성 완료 클릭
        $("#write-board").click(function(event){
            event.preventDefault();
...
// 게시글 내용 보기 뷰 세팅
function viewFreeBoardDetail(result){
    // 게시글 내용 테이블 뷰
    freeBoardView.renderFreeBoardDetail(result); //게시물 내용영역 뷰 세팅
    ...
    $(document).off('click', '.anonymous-link-edit').on('click', '.anonymous-link-edit', function(event){
        ...
        $("#confirm-reply-password").off('click').on('click', function(event){
            ...
            if(!inputPassword){
                ...
                var ref = consts.freeBoardRef.child(result[0].idx).child('reply').child(value).child('replyPassword');
                ref.once('value', function(snapshot){
                    if(snapshot.exists()){
    ...
    $("#confirm-pw-edit-post").click(function(event){
        ...
        if(result[0].password){ 
            ...
            if(!password){
                ...
                editPost(result);
                var url = "/board/free/detail/edit/" + result[0].idx;
    ...
    // 댓글 작성 완료 클릭
    $("#write-reply").click(function(event){
        event.preventDefault();
...
function editReply(postKey, replyKey){
    ...
    $("#edit-reply-ok").off('click').on('click', function(event){
        ...
        boardDAO.updateValueReply(consts.freeBoardRef, editData, function(){
            $("#" + replyKey).children().show();
            $("#reply-edit-area").remove();
            freeBoardDetail(editData.postKey);
        });
    });
...
function removeReply(boardKey, replyKey, type, inputPassword){
    ...
    $("#delete-reply").unbind("click").bind("click",function(){
        ...
        var ref = consts.freeBoardRef.child(boardKey).child('reply').child(replyKey);
        switch(type){
            case 'auth':
                ref.remove().then(function(){
                    freeBoardDetail(boardKey);
                    console.log('댓글삭제 성공');     
...
// 자유게시판 페이지로 리다이렉트
function redirectBoardFree(){
    $(document).ready(function(){
        $("#board-free-title").click(function(event){
            event.preventDefault();
...
function editPost(result){
    ...
    var editPostArea = freeBoardView.rednerEditPostSummernote(result);
...
//게시글 DB세팅
function setValueFreeBoard(title, textWithTag){ 
    var writeRef = consts.freeBoardRef.push(); 
    boardDAO.setValuePost(title, textWithTag, writeRef);
}

//게시글 DB수정
function setValuePostData(key, node, value){
    var ref = consts.freeBoardRef.child(key).child(node);
    boardDAO.updateValuePost(ref, value);
}

//게시글 DB삭제
function deletePost(key, isAuth){
    var ref = consts.freeBoardRef.child(key);
    boardDAO.deleteValuePost(ref).then(function(isDeleteSuccess){
        if(isDeleteSuccess){
            if(isAuth){
                ...
                $("#write-reply").prop('disabled', true);
            }else{
...
function setValueReply(key, textWithTag, anonymousNickname, anonymousPassword){
    boardDAO.setValueReply(consts.freeBoardRef, key, textWithTag, anonymousNickname, anonymousPassword, function(postKey){
        if(postKey){
            freeBoardDetail(postKey);
        }
    });    
}

//조회수 업데이트 트랜잭션 
function viewsTransaction(key){
    var ref = consts.freeBoardRef.child(key);
    var viewsRef = consts.freeBoardRef.child(key).child('views');
    boardDAO.updateViews(ref, viewsRef);
}

//자유게시판 더 보기 데이터 요청
function freeBoard(ref){
    boardDAO.getBoardMore(ref).then(function(data){
        if(!data.result.length){
            viewFreeBoard("", false, "", "", "paging");  
        }else{
            viewFreeBoard(data.result, false, data.recentKey, "paging");
        }
    });
}

//게시글 내용 보기 데이터 요청
function freeBoardDetail(key){
    var ref = firebase.database().ref('board').child('free').child(key);
    boardDAO.getBoardDetail(ref).then(function(result){
        if(result.length > 0){
            viewFreeBoardDetail(result);
            utills.setPageHistory({data: key, url: "/board/free/detail?postKey=" + key}, "/board/free/detail?postKey=" + key); 
        }else{
            alert('삭제된 게시글 입니다.');
        }
    });  
...
//자유게시판 페이징 처리 데이터 요청
function pagination(page, key){
    switch(page){
        case 'forward':
            var ref = firebase.database().ref('board').child('free').orderByKey().endAt(key).limitToLast(consts.postCountPerPage + 1);
            paginator(ref, 'forward');
            break;
        case 'backward': 
            var ref = firebase.database().ref('board').child('free').orderByKey().startAt(key).limitToFirst(consts.postCountPerPage + 1);
            paginator(ref, 'backward');
            break;
        default:
...
function paginator(ref, type){
    boardDAO.paging(ref, type).then(function(data){
        if(data){
            viewFreeBoard(data.result, data.isLastPage, data.recentKey, "paging");  //추가 페이지 로드
        }else{
...
export { redirectBoardFree, freeBoard, getsummernote, freeBoardDetail, pagination, viewsTransaction, viewFreeBoard }