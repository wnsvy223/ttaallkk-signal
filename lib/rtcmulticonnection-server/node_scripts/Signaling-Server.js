        ...
        function broadcastPublicRoomOpen(roomid) { 
            try {
                ...
                if(room && room.identifier !== undefined && room.identifier === 'publicRoomIdentifier') {
                    publicRooms.length = []; // clear public rooms array
                    Object.values(listOfRooms).forEach(function(publicRoom){
                        publicRooms.push({
                            maxParticipantsAllowed: publicRoom.maxParticipantsAllowed,
                            owner: publicRoom.owner,
                            participants: publicRoom.participants,
                            extra: publicRoom.extra,
                            session: publicRoom.session,
                            sessionid: publicRoom.sessionid,
                            isRoomFull: publicRoom.participants.length >= publicRoom.maxParticipantsAllowed,
                            isPasswordProtected: !!publicRoom.password && publicRoom.password.replace(/ /g, '').length > 0
                        });
                    });
